...
public abstract class ReflectionUtils {
	...
	public static Method[] getUniqueDeclaredMethods(Class<?> leafClass) throws IllegalArgumentException {
		final Map<MethodIdentifier,Method> methods = new HashMap<MethodIdentifier, Method>();
		doWithMethods(leafClass, new MethodCallback() {
			public void doWith(Method method) {
				boolean knownSignature = false;
				MethodIdentifier methodBeingOverriddenWithCovariantReturnType = null;
				MethodIdentifier identifier = new MethodIdentifier(method.getName(), method.getParameterTypes());
					if(methods.containsKey(identifier)){
						Method existingMethod = methods.get(identifier);
						// is this a covariant return type situation?
						if (existingMethod.getReturnType() != method.getReturnType() &&
								existingMethod.getReturnType().isAssignableFrom(method.getReturnType())) {
							methodBeingOverriddenWithCovariantReturnType = identifier;
						} else {
							knownSignature = true;
						}
					}
