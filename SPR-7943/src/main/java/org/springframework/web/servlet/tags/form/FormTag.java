...
public class FormTag extends AbstractHtmlElementTag {
	...
	/** ParameterHandler that has been bound to the request by the DispatcherServlet */
	...
	protected int writeTagContent(TagWriter tagWriter) throws JspException {
		...
		parameterHandler = RequestContextUtils.getParameterHandler(request);
		
		tagWriter.startTag(FORM_TAG);
		writeDefaultAttributes(tagWriter);
		
		String resolvedAction = resolveAction();
		parameterHandler.processStartForm(request, resolvedAction);
		tagWriter.writeAttribute(ACTION_ATTRIBUTE, resolvedAction);
		...
		if (!isMethodBrowserSupported(getMethod())) {
			...
			writeOptionalAttribute(tagWriter, NAME_ATTRIBUTE, 
					parameterHandler.processFormParameterName(request, getMethodParameter(), "hidden"));
			writeOptionalAttribute(tagWriter, VALUE_ATTRIBUTE, 
					parameterHandler.processFormParameterValue(request, getMethodParameter(), getMethod(), "hidden"));
	...
	public int doEndTag() throws JspException {
		Hashtable<String, String> extraParameters = 
			(Hashtable<String, String>) this.parameterHandler.getExtraFormParameters();
		
		if (extraParameters != null) {			
			for (String name : extraParameters.keySet()) {
				this.tagWriter.startTag(INPUT_TAG);
				writeOptionalAttribute(this.tagWriter, TYPE_ATTRIBUTE, "hidden");
				writeOptionalAttribute(this.tagWriter, NAME_ATTRIBUTE, name);
				writeOptionalAttribute(this.tagWriter, VALUE_ATTRIBUTE, extraParameters.get(name));
